...
figure;
    ax1 = subplot(121);
    [CC,jsf,im] = plot_contours(A(:,keep),Cn,options,0,[],Coor_k,'m',find(keep)); title('Selected components','fontweight','bold','fontsize',14);
...
roi_ave.C_full = imresize(C_keep,[N,T]);                % upsample to original frame rate
R_full = imresize(R_keep,[N,T]);                % upsample to original frame rate
roi_ave.F_full = C_full + R_full;                       % full fluorescence
...
for i = 1:numFiles
    ...
    [roi_ave.C_full(:,inds),f_full(:,inds),~,~,roi_ave.R_full(:,inds)] = update_temporal_components_fast(h5_files(i).name,A_keep,b,C_full(:,inds),f_full(:,inds),P,options);
...
[roi_ave.F_dff,F0] = detrend_df_f(A_keep,b,C_full,f_full,R_full,options);

roi_ave.C_dec = zeros(N,T);         % deconvolved DF/F traces
roi_ave.S_dec = zeros(N,T);         % deconvolved neural activity
roi_ave.bl = zeros(N,1);            % baseline for each trace (should be close to zero since traces are DF/F)
roi_ave.neuron_sn = zeros(N,1);     % noise level at each trace
...
for i = 1:N
    ...
    roi_ave.bl(i) = opts_oasis.b;
    roi_ave.C_dec(i,:) = cc(:)' + bl(i);
    roi_ave.S_dec(i,:) = spk(:);
    roi_ave.neuron_sn(i) = opts_oasis.sn;
    g{i} = opts_oasis.pars(:)';
