...
for i = 1:size(ROIhits_s,3);  
    GG(:,i) = abs( max((smooth(mean(ROIhits_s(:,:,i)),10)))- min((smooth(mean(ROIhits_s(:,:,i)),10))));
end
...
hold on
    plot(smooth(mean(ROIhits_s(sorting(:),:,bx(i))),10),'r')%./(smooth(var(ROIhits_s(:,:,bx(i))),10)+.1));
  ...
  bFig(2) = subplot(10,1,3:10); 
    imagesc(ROIhits_s(sorting,:,bx(i))); 
    linkaxes(bFig,'x'); 
