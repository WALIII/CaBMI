...
% WAL3
function [roi_ave1, roi_ave2, roi_ave3, roi_ave4, ROIa, ROIb, ds_hits, roi_hits, ROIhits, ROIhits_d, ROIhits_s, ROIhits_z ,D_ROIhits, D_ROIhits_d, D_ROIhits_s, D_ROIhits_z] =  CaBMI_Figure_Generator;
...
if ismember(1,figs)
 out.spacetime =  CaBMI_Spatiotemporal(ROIhits, ROIa,ROIb);
cd(pwd_here);
cd('..')
...
if ismember(7,figs);
    [out.dim_explained] = CaBMI_FA(ROIhits_z,roi_ave1);
    % TO DO: Save data
cd(pwd_here);
cd('..')
if exist('DATA_dim')<1;
...
[out.PCAconsistancy] = CaBMI_PCA_consistancy(PCA_hits);
    % TO DO: Save data
cd(pwd_here);
cd('..')
if exist('DATA_con')<1;
