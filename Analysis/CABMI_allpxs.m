...
for i=1:2:nparams
    switch lower(varargin{i})
        ...
        case 'type'
            type = varargin{i+1};
        case 'long_mov'
            long_mov = varargin{i+1};
        case 'mask2use'
            mask2use = varargin{i+1};
        case 'scaling_on'
            scaling_on = varargin{i+1};
    end
end
...
if strcmp(type,'max')
    max_proj=max(dff,[],3);
elseif strcmp(type,'ccimage')
    disp('computing cc image')
    [max_proj] = CrossCorrImage(dff);
elseif strcmp(type,'std')
    max_proj=std(dff,[],3);
end

if exist('mask2use')>0
    max_proj = mask2use.^exp;
end
%
