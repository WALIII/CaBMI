...
mkdir([START_DIR_ROOT,'\','Processed','\',subFolders(i).name,S2S(ii).name]);
% Extracting CSV
if csv_ext ==1;
disp('Saving CSV extraction...');
try
[csv_data] = CaBMI_csvExtract;
save([START_DIR_ROOT,'\','Processed','\',subFolders(i).name,S2S(ii).name,'\','csv_data'],'csv_data','-v7.3');
catch
...
save([START_DIR_ROOT,'\','Processed','\',subFolders(i).name,S2S(ii).name,'\','Y.mat'],'Y','-v7.3');
end
% copy data over
disp('copying data...');
copyfile([S2S(ii).folder,'\',S2S(ii).name,'\','Processed\','roi\','ave_roi.mat'],[START_DIR_ROOT,'\','Processed','\',subFolders(i).name,S2S(ii).name]);
copyfile([S2S(ii).folder,'\',S2S(ii).name,'\','Processed\Mtiff_folder2\','Direct_neuron_roi\','Direct_roi.mat'],[START_DIR_ROOT,'\','Processed','\',subFolders(i).name,S2S(ii).name]);
...
% copy data over
disp('copying data...');
mkdir([START_DIR_ROOT,'\','Processed','\',subFolders(i).name,S2Sm(iii).name]);
copyfile([S2Sm(iii).folder,'\',S2Sm(iii).name,'\','Processed\Mtiff_folder2\','roi\','Indirect_roi.mat'],[START_DIR_ROOT,'\','Processed','\',subFolders(i).name,S2Sm(iii).name]);
copyfile([S2Sm(iii).folder,'\',S2Sm(iii).name,'\','Processed\Mtiff_folder2\','Direct_neuron_roi\','Direct_roi.mat'],[START_DIR_ROOT,'\','Processed','\',subFolders(i).name,S2Sm(iii).name]);
